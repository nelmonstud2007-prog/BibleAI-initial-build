...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    await supabase.from("sanctuary_audit_logs").insert({
      user_id: user.id,
      action: "bible_chat_request",
      metadata: { conversation_id, persona, message_length: messages[messages.length - 1]?.content?.length },
      ip_address: req.headers.get("x-real-ip") || req.headers.get("x-forwarded-for")
    });

    const { data: profile } = await supabase.from("profiles").select("subscription_tier, full_name").eq("id", user.id).maybeSingle();
    ...
    if (conversation_id) {
      await supabase.from("chat_messages").insert([
        ...
        { user_id: user.id, conversation_id, role: "assistant", content }
      ]);
