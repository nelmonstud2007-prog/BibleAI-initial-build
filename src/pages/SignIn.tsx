...
export default function SignIn() {
  const { user, profileCompleted, signIn } = useAuth();
      ...
      <div className="relative hidden lg:flex overflow-hidden border-r border-navy-800">
        ...
        <div className="relative z-10 p-12 flex flex-col justify-between w-full">
          ...
          <div className="max-w-md">
            ...
            <p className="mt-4 text-gold-300 font-medium">Proverbs 3:5</p>
          </div>
          <p className="text-xs text-navy-400">Walk in the Word with wisdom, one day at a time.</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center px-4 sm:px-8 py-10">
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            ...
            <p className="mt-2 text-sm text-navy-300">Sign in to continue your faith journey</p>
          </div>
          <form onSubmit={handleSubmit} className="bg-navy-900/60 border border-navy-800 rounded-2xl p-6 sm:p-8 space-y-5 shadow-2xl shadow-black/30">
            {error && (
              <div className="bg-red-400/10 border border-red-400/20 rounded-xl px-4 py-3 text-sm text-red-400">{error}</div>
            )}
            <button type="button" onClick={handleGoogleSignIn} disabled={googleLoading}
              ...
              {googleLoading ? (
                <span className="inline-flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" />Redirecting...</span>
              ) : (
                <span className="inline-flex items-center gap-2"><FcGoogle className="w-5 h-5" />Continue with Google</span>
              )}
            </button>
            ...
            <p className="text-center text-sm text-navy-400">
              ...
              <Link to="/signup" className="text-gold-400 hover:text-gold-300 font-medium transition-colors">Sign up</Link>
            </p>
          </form>
        </div>
      </div>
